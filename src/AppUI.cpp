...
void AppUI::RefreshDevices() {
    ...
    std::wstring lowerTarget = targetToFind;
    std::transform(lowerTarget.begin(), lowerTarget.end(), lowerTarget.begin(), ::towlower);

    for (int i = 0; i < (int)m_devices.size(); ++i) {
        std::wstring lowerDev = m_devices[i].friendlyName;
        std::transform(lowerDev.begin(), lowerDev.end(), lowerDev.begin(), ::towlower);
...
// Pixel-perfect Fluent Toggle Switch
bool AppUI::DrawToggleSwitch(const char* id, bool* v) {
    ...
    if (ImGui::IsItemHovered())
        col_bg = *v ? IM_COL32(95, 110, 245, 255) : IM_COL32(65, 70, 88, 255);
    else
        col_bg = *v ? IM_COL32(80, 95, 235, 255) : IM_COL32(48, 52, 66, 255);
...
void AppUI::Render() {
    ...
    // ==========================================
    // 1. HEADER & MASTER SWITCH CARD (Height: 64px)
    // ==========================================
    {
        ...
        if (!m_isGuardEnabled) {
            ...
            ImGui::SameLine();
            std::string devName = WStringToUtf8(m_pMuter->GetActiveDeviceName());
    ...
    // ==========================================
    // 2. AUDIO OUTPUT DEVICE (Height: 68px)
    // ==========================================
    {
        ...
        std::string currentItemPreview = (m_selectedDeviceIndex >= 0 && m_selectedDeviceIndex < (int)m_devices.size()) 
            ? WStringToUtf8(m_devices[m_selectedDeviceIndex].friendlyName) 
            : "Select audio device...";
        ...
        if (ImGui::BeginCombo("##AudioDevicesCombo", currentItemPreview.c_str())) {
            for (int i = 0; i < (int)m_devices.size(); i++) {
                ...
                std::string devName = WStringToUtf8(m_devices[i].friendlyName);
    ...
    // ==========================================
    // 3. APPLICATIONS (Unified Clean Interactive List, Height: 150px)
    // ==========================================
    {
        ...
        if (m_pMuter) {
            auto activeSessions = m_pMuter->GetActiveSessions();
            ...
            // 2. Add configured targets that might not be currently running
            for (const auto& target : targetList) {
                bool alreadyInList = false;
                for (const auto& d : displayList) {
                    std::wstring lowerA = d.name;
                    std::wstring lowerB = target;
        ...
        if (displayList.empty()) {
            ...
            for (size_t i = 0; i < displayList.size(); ++i) {
                ...
                std::string pName = WStringToUtf8(app.name);
        ...
        if (ImGui::Button("+ Add", ImVec2(64, 0))) {
            ...
            if (!appStr.empty()) {
                if (m_pMuter) {
                    m_pMuter->AddTargetProcess(Utf8ToWString(appStr));
                    SaveConfigFromUI();
                }
    ...
    // ==========================================
    // 4. SETTINGS CARD (Dedicated Sleek Bar, Height: 44px)
    // ==========================================
    {
    ...
    // ==========================================
    // 5. RECENT ACTIVITY CARD (Spacious & Tall Log Box)
    // ==========================================
    {
        ...
        // Log Content Box (Fills entire rest of card with plenty of room!)
